...
/** Small icon + name row, with graceful fallback if the Digimon isn't in the list. */
function DigimonChip({ name, digimonList }) {
  const found = digimonList.find(d => d.name === name);
  return (
...
function DnaPathCard({ conditions, digimonList }) {
  return (
      ...
      <div className="space-y-2">
        {conditions.map((c, i) => (
          <div key={i} className="flex items-center justify-between gap-3">
            <DigimonChip name={c.digimon} digimonList={digimonList} />
            <span className="text-xs px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-800 text-purple-700 dark:text-purple-300 whitespace-nowrap flex-shrink-0">
              {c.requirement}
            </span>
          </div>
        ))}
...
function ModeChangeCard({ from, digimonList }) {
  return (
    <div className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4">
      ...
      <div className="flex items-center gap-2">
        <span className="text-sm text-gray-600 dark:text-gray-400 flex-shrink-0">From</span>
        <DigimonChip name={from} digimonList={digimonList} />
      </div>
    </div>
  );
}

function EvolutionPaths({ paths, digimonList }) {
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {paths.map((path, i) => {
          if (path.type === 'dna')
            return <DnaPathCard key={i} conditions={path.conditions ?? []} digimonList={digimonList} />;
          if (path.type === 'mode_change')
            return <ModeChangeCard key={i} from={path.from} digimonList={digimonList} />;
          return null;
        })}
      </div>
    </div>
...
function RequirementsSection({ requirements, digimonList }) {
  if (!requirements) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      {hasBase && hasPaths && <hr className="border-gray-200 dark:border-gray-700" />}
      {hasPaths && <EvolutionPaths paths={requirements.paths} digimonList={digimonList} />}
      {!hasBase && !hasPaths && (
        <p className="text-sm text-gray-500 dark:text-gray-400 italic">
          This Digimon has no evolution requirements.
        </p>
...
export function DigimonModal({ digimon, onClose, digimonList = [] }) {
  const [isImageOpen, setIsImageOpen] = useState(false);
  const [imageError, setImageError] = useState(false);

  // Reset state whenever the modal opens for a different Digimon
  useEffect(() => {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex justify-center">
              {digimon.image && !imageError ? (
                <div className="relative group">
                  <img
                    src={digimon.image}
                    alt={digimon.name}
                    className="max-h-56 w-auto rounded-2xl shadow-md object-contain cursor-zoom-in transition-opacity group-hover:opacity-90"
            ...
            <div className="bg-gray-50 dark:bg-gray-700/40 rounded-2xl p-5">
              ...
              <RequirementsSection requirements={digimon.requirements} digimonList={digimonList} />
            </div>
